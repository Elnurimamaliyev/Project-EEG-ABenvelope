...
for s=1:length(sbj)
    ...
    %% load the set files from the previous script
    %setup path file
    PATH = fullfile([DATAPATH '\data\'], sb,filesep); %is not in the setup script, since it changes dynamically with the participants
    cd(PATH)
    [ALLEEG EEG CURRENTSET ALLCOM] = eeglab('nogui');
    % file = {'narrow_game_added_trigger.set','wide_game_added_trigger.set'};  %set up name
    file = {'narrow_game_added_trigger.set'};  %set up name
    ...
    %save the removed channel information -> needed later when we add the
    %ICA weights to the raw data again
    if isfield(EEG.chaninfo,'removedchans')
        badchan = EEG.chaninfo.removedchans;
        chan_rej(s,1) = size(badchan,2); %for me to report later how many bad channels were rejected
    else
        chan_rej(s,1) = 0;
        
    end
    
    %reference the electrodes to average 
    EEG = pop_reref(EEG,[]);
   
    
    %run ICA
    EEG = eeg_regepochs(EEG, 'recurrence', 1);      %epoch data
    EEG = eeg_checkset(EEG, 'eventconsistency');    %sanity check if everything has worked so far
    EEG.data = double(EEG.data);                    %double precision never hurts
    
    % remove epochs with artefacts to improve ICA training
    PRUNE = 3;
    EEG = pop_jointprob(EEG, 1, [1:size(EEG.data,1)], PRUNE, PRUNE, 0, 1, 0); %artifactual epochs are rejected
    EEG = eeg_checkset(EEG, 'eventconsistency');                              %sanity check
