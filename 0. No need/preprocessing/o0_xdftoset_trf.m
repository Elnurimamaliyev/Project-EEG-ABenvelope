% we need to transform the XDF files into SET files such that they can be
% used by EEGLAB 

o0_setupscript_trf   %start by intializing your setup file -> all important paths will be set

%%
for s = 1:length(sbj) %loops through all participants
    ...
    for k = 1:length(task) %loop over the two conditions as two EEG sets exist
...
%         audio_strct = pop_loadxdf([raw_data_path,sbj{s},'\sub-',sbj{s},'_ses-S001_task-',task{k},'_run-001_eeg.xdf'],...
...
%         
        ...
        if sum(ismember({EEG.event.type}, 'eyes_closed_end')) > 0 && sum(ismember({EEG.event.type}, 'eyes_closed_start')) > 0
            ...
            %eyes closed
            filename = [task{k},'_eyes_closed'];           %set the filename       
            EEG_eyesc.setname = [sbj{i},'_',filename];     %set the set name in the EEG structure
            pop_saveset(EEG_eyesc, 'filename',filename);   %save the set
            
        end
        %open 
        if sum(ismember({EEG.event.type}, 'eyes_open_end')) > 0 && sum(ismember({EEG.event.type}, 'eyes_open_start')) > 0
            %open
            %find the duration of the eyes open condition 
    ...
    cd('..') %go bvack to the main data path 
end